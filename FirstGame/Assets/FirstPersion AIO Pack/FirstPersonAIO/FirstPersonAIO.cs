...
public class BETA_SETTINGS{
    ...
    public void adjustSensitivity(float sensitivityMultiplier)
    {
        mouseSensitivity = defaultSensitivity + sensitivityMultiplier * defaultSensitivity;
    }
...
/*     public IEnumerator FOVKickOut()
    ...
    float SlopeCheck(){
            ...
            float returnValue = new AnimationCurve(new Keyframe(-90.0f, 1.0f), new Keyframe(0.0f, 1.0f), new Keyframe(advanced._maxSlopeAngle, 0.0f), new Keyframe(90, 0.0f)) { preWrapMode = WrapMode.Clamp, postWrapMode = WrapMode.ClampForever }.Evaluate(advanced.lastKnownSlopeAngle);
            if (returnValue == 0) return 1f;
            return returnValue;
    }
