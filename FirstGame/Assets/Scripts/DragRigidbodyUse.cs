...
public class DragRigidbodyUse : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		if (Input.GetButton(GrabButton) && PlayerVariables.emptyHands)
		{
			if (!isObjectHeld)
			{
				tryPickObject();
	...
	{
		objectHeld.GetComponent<Rigidbody>().AddForce(playerCam.transform.forward * ThrowStrength * objectHeld.GetComponent<Rigidbody>().mass);
		objectHeld.GetComponent<Rigidbody>().freezeRotation = false;
		objectHeld = null;
	}
